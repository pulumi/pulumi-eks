...
export function evaluatePlatform(facts: PlatformFacts): CompatibilityRisk[] {
            ...
            ? [
                  {
                      ...
                      message: `This operating system is below the Node.js ${
                          ...
                          facts.platform === "darwin" ? "macOS" : "Windows"
                      } ${minimum}).`,
                  },
              ]
            : [];
    ...
    if (facts.distribution === "alpine") {
        ...
        const comparison = minimum
            ? compareVersions(facts.distributionVersion, minimum)
            : undefined;
        if (comparison === undefined) {
            risks.add(unknownRisk);
...
export function formatCompatibilityWarning(risks: CompatibilityRisk[]): string {
    return (
        `[eks:node-compatibility:${risks.map((risk) => risk.reason).join(",")}] ` +
        "A future EKS major version may use a dynamically linked stock Node.js runtime. " +
...
export const warnNodeCompatibility = createCompatibilityWarning({
    suppressed: () => new pulumi.Config("eks").get("suppressNodeCompatibilityWarning") === "true",
