...
it("warns for Alpine without assuming that the packaged static runtime identifies the host libc", () => {
    const risks = evaluatePlatform({
        ...linux,
        distribution: "alpine",
        distributionVersion: "3.22",
    });
...
it("reports multiple known risks in a single warning and does not duplicate unknown reasons", () => {
    expect(
        evaluatePlatform({ ...linux, release: "3.10", distribution: "alpine" }).map(
            (r) => r.reason,
        ),
...
it("includes a stable reason, feedback link, and suppression command", () => {
    expect(formatCompatibilityWarning(evaluatePlatform({ ...linux, distribution: "nixos" }))).toBe(
        "[eks:node-compatibility:unknown] A future EKS major version may use a dynamically linked stock Node.js runtime. " +
            "EKS could not determine whether this environment meets the requirements of a future stock Node.js runtime. " +
            `To provide feedback see ${nodeRuntimePolicy.feedbackUrl}. ` +
            "To suppress this warning, run `pulumi config set eks:suppressNodeCompatibilityWarning true`.",
    );
});
...
describe("warning delivery", () => {
    ...
    it("converts unexpected probe failures into an unknown warning", async () => {
        ...
        await warning.check();
        expect(warning.warn).toHaveBeenCalledWith(
            expect.stringContaining("[eks:node-compatibility:unknown]"),
        );
    });
    ...
    it("reads the suppression setting from Pulumi stack configuration", async () => {
        pulumi.runtime.setAllConfig({ "eks:suppressNodeCompatibilityWarning": "true" });
