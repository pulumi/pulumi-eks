  ...
  try {
    const port = await new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(
          new Error(
            `provider did not print a port within 20 seconds; stdout=${JSON.stringify(
              stdout
      ...
      child.once("exit", (code, signal) => {
        if (!settled) {
          ...
          reject(
            new Error(
              `provider exited before printing a port: code=${code} signal=${signal}; stdout=${JSON.stringify(
                stdout
              )} stderr=${JSON.stringify(stderr)}`
            )
          );
...
const timings = results
  .map((result) => result.startupMilliseconds)
  .sort((a, b) => a - b);
