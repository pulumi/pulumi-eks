...
func TestAccClusterUpgrade(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping test in short mode.")
	}

	eksClient := createEksClient(t)
	k8sVersions, err := utils.ListKubernetesVersions(eksClient)
	require.NoError(t, err)
	require.GreaterOrEqual(t, len(k8sVersions), 2, "expected at least 2 k8s versions to be available")

	initialVersion := k8sVersions[1]
	upgradeVersion := k8sVersions[0]

	test := getJSBaseOptions(t).
		With(integration.ProgramTestOptions{
			Dir: path.Join(getCwd(t), "tests", "cluster-upgrade"),
			Config: map[string]string{
			...
			ExtraRuntimeValidation: func(t *testing.T, info integration.RuntimeValidationStackInfo) {
				...
				clusterName := info.Outputs["clusterName"].(string)
				nodeGroupName := info.Outputs["nodeGroupName"].(string)

				mng, err := utils.GetEksNodeGroup(t, eksClient, clusterName, nodeGroupName)
				require.NoError(t, err)
				assert.Equal(t, initialVersion, *mng.Version, "expected nodegroup to be on initial version %q", initialVersion)
			},
			// step2 upgrades the cluster to the latest version
			EditDirs: []integration.EditDir{
				{
					Dir:      path.Join(getCwd(t), "tests", "cluster-upgrade", "step2"),
					Additive: true,
					ExtraRuntimeValidation: func(t *testing.T, info integration.RuntimeValidationStackInfo) {
						utils.RunEKSSmokeTest(t,
							info.Deployment.Resources,
						...
						mng, err := utils.GetEksNodeGroup(t, eksClient, clusterName, nodeGroupName)
						require.NoError(t, err)
						assert.Equal(t, upgradeVersion, *mng.Version, "expected nodegroup to be on the upgraded version %q", upgradeVersion)
					},
				},
			},
		})
